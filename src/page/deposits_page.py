...
def get_market_symbol(market_index: int) -> str:
    ...
    try:
        ...
        if market_index not in valid_indices:
            logger.error(
                f"Invalid market index: {market_index}. Valid indices: {valid_indices}"
            )
...
def deposits_page():
    try:
        ...
        if radio_option == "All":
            ...
            if market_index not in valid_indices:
                logger.warning(
                    f"Invalid market index in params: {market_index}. Defaulting to first available market."
                )
            ...
            with col2:
                market_index = st.selectbox(
                    "Market index",
                    valid_indices,
                    index=valid_indices.index(market_index)
                    if market_index in valid_indices
                    else 0,
        ...
        logger.info(
            f"Fetching deposits for market_index: {market_index if radio_option == 'By Market' else 'All'}"
        )

        if radio_option == "All":
            result = fetch_api_data(
                "deposits", "deposits", params={"market_index": None}
            )
        else:
            result = fetch_api_data(
                "deposits", "deposits", params={"market_index": market_index}
            )
        ...
        with tabs[1]:
            ...
            grouped_df: pd.DataFrame = grouped_df.loc[
                grouped_df["value"] >= min_balance
            ].copy()
            st.write(f"Total deposits value: **${grouped_df['value'].sum():,.2f}**")
            st.write(
                f"Total number of authorities with deposits: **{len(grouped_df):,}**"
            )
