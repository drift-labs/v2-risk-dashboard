...
from driftpy.addresses import (
    get_high_leverage_mode_config_public_key,  # To get the PDA for config
)
from driftpy.constants.numeric_constants import (  # Added QUOTE_PRECISION
    BASE_PRECISION,
    MARGIN_PRECISION,
    PRICE_PRECISION,
    QUOTE_PRECISION,
...
from driftpy.constants.perp_markets import (
    mainnet_perp_market_configs,  # Added mainnet_perp_market_configs
)
...
from driftpy.types import (  # Added OraclePriceData
    OraclePriceData,
    PerpPosition,
    UserAccount,
    is_variant,
...
async def get_high_leverage_stats(request: BackendRequest):
    ...
    drift_client: Optional[DriftClient] = getattr(
        request.state.backend_state, "dc", None
    )
    ...
    if drift_client:
        try:
            ...
            ].fetch(high_leverage_mode_config_pda)
            total_spots = config_account.max_users
            opted_in_users_count = config_account.current_users
            logger.info(
                f"Successfully fetched HighLeverageModeConfig: max_users={total_spots}, current_users={opted_in_users_count}"
            )
        except Exception as e:
            logger.error(
                f"Error fetching HighLeverageModeConfig from on-chain: {e}. Using fallback values for total and opted-in spots.",
                exc_info=True,
            )
            # Fallback values initialized earlier will be used.
    else:
        logger.warning(
            "DriftClient (as 'dc') not found in backend state. Using fallback values for total and opted-in spots."
        )
        # Fallback values initialized earlier will be used.
    ...
    if not user_map or not hasattr(user_map, "values"):
        logger.warning(
            "UserMap not found or invalid in backend state. Bootable spots count will be 0."
        )
        ...
        if current_slot == 0:
            logger.warning(
                "Could not retrieve current_slot (last_oracle_slot) from backend state for bootable check. Bootable count might be inaccurate (0)."
            )
            ...
            try:
                ...
                logger.info(
                    f"Processing {len(user_values)} users from UserMap for bootable status calculation."
                )

                for user in user_values:
                    if not isinstance(user, DriftUser):
                        logger.warning(
                            f"Skipping item in user_map values for bootable calc, expected DriftUser, got {type(user)}"
                        )
                        continue
                    ...
                    try:
                        ...
                        if (
                            is_high_leverage
                        ):  # Only proceed if user is in high leverage mode
                            user_account = user.get_user_account()
                    except Exception as e:
                        logger.error(
                            f"Error checking high leverage status or getting account for user {user.user_public_key} in bootable calc: {e}",
                            exc_info=True,
                        )
                        continue
                    ...
                    if is_high_leverage and user_account:
                        ...
                        try:
                            ...
                            if (
                                current_slot - last_active_slot_int
                            ) > SLOT_INACTIVITY_THRESHOLD:
                                is_inactive = True
                                logger.debug(
                                    f"User {user.user_public_key} is inactive for bootable check. Current: {current_slot}, Last Active: {last_active_slot_int}, Diff: {current_slot - last_active_slot_int}"
                                )
                        except Exception as slot_check_e:
                            logger.error(
                                f"Error checking inactivity for user {user.user_public_key} (bootable calc): {slot_check_e}",
                                exc_info=True,
                            )
                ...
                logger.error(
                    f"Error getting or processing users from UserMap for bootable calculation: {e}",
                    exc_info=True,
                )
    ...
    for user in user_values:
        ...
        try:
            if user.is_high_leverage_mode():
                high_leverage_count = (
                    getattr(request.state.backend_state, "high_leverage_count", 0) + 1
                )
                setattr(
                    request.state.backend_state,
                    "high_leverage_count",
                    high_leverage_count,
                )
                logger.info(
                    f"Found high leverage mode user: {user.user_public_key} (Total: {high_leverage_count})"
                )
                ...
                logger.debug(
                    f"User {user_public_key_str} is high leverage. Account Leverage: {account_leverage_ui:.2f}x (raw: {account_leverage_raw})"
                )
                ...
                for position in user_account.perp_positions:
                    ...
                    try:
                        ...
                        base_asset_amount_ui = base_asset_amount_val / (
                            10**PERP_DECIMALS
                        )
                        ...
                        if margin_requirement_raw != 0:
                            ...
                            position_leverage_ui = (
                                position_leverage_raw / MARGIN_PRECISION
                            )
                        else:
                        ...
                        if market_index < len(mainnet_perp_market_configs):
                            market_symbol = mainnet_perp_market_configs[
                                market_index
                            ].symbol
                        else:
                            logger.warning(
                        ...
                        ValueError,
                        AttributeError,
                    ) as calc_e:  # Added AttributeError for safety
                        logger.error(
                            f"Error calculating position data or leverage for user {user_public_key_str}, market {market_index}: {calc_e}. Skipping.",
                            exc_info=True,
                        )
                        continue
        except Exception as user_proc_e:
            logger.error(
                f"Error processing user {getattr(user, 'user_public_key', 'UNKNOWN')} for /positions/detailed: {user_proc_e}",
                exc_info=True,
            )
    ...
    for user in user_values:
        ...
        try:
            if user.is_high_leverage_mode():
                ...
                if not user_account:
                    logger.warning(
                        f"User {user.user_public_key} is high leverage but failed to get user_account. Skipping."
                    )
                    continue
                ...
                if activity_staleness_slots > SLOT_INACTIVITY_THRESHOLD:
                    ...
                    total_collateral_usd = (
                        user.get_total_collateral(MarginCategory.INITIAL)
                        / QUOTE_PRECISION
                    )
            ...
            logger.error(
                f"Error processing user {getattr(user, 'user_public_key', 'UNKNOWN')} for /bootable-users: {user_proc_e}",
                exc_info=True,
            )
            continue
